...
let handler = async (m, { conn, args }) => {
    ...
    if (!who) return m.reply(await conn.trans(lang, '@tag atau balas pesan user').catch(async _ => await conn.trans2(lang, '@tag atau balas pesan user')))
    if (!args) return m.reply(let capt = await conn.trans(lang, 'Angkanya mana?').catch(async _ => await conn.trans2(lang, 'Angkanya mana?')))
