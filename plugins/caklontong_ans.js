...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.caklontong[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Jawaban Benar!*\n\nSelamat @${m.sender.split`@`[0]} jawaban kamu benar\n+${this.caklontong[id][2]} XP\n${json.deskripsi}`, wm, false, [['Cak Lontong', '.caklontong']], m, 0, { contextInfo: { mentionedJid: [m.sender] } })
            clearTimeout(this.caklontong[id][3])
            delete this.caklontong[id]
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) conn.reply(m.chat, `*Dikit Lagi!*`, m)
