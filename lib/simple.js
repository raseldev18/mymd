...
END:VCARD`.trim()
    ...
    */
    ...
    conn.replys = async(jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text: await tr(text, {from: 'id', to: global.lang}).catch(async _=> await tr2(text, {from: 'id', to: global.lang})) }, { quoted, ...options })
    }
  ...
  */
    ...
    conn.sendTBVG = async (jid, contentText, footer, video, dtux, urlx, dtcx, nmbrx, buttons1, row1, buttons2, row2, buttons3, row3, quoted, options) => {
        const message = {
            video: { url: video }, ...options,
            gifPlayback: true, 
            //jpegThumbnail: await (await fetch(thumbt)).buffer(), fileLength: fsx,
            caption: contentText,
            footer: footer,
            templateButtons: [
                {
...
exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        m.fromOwner = [conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.fromOwner = [conn.user.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
