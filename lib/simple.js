...
const language = global.langx || 'en'
...
END:VCARD`.trim()
    ...
    */
    ...
    conn.reply = async (jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text: await tr(text || quoted, {from: 'auto', to: language}) }, { quoted, ...options })
    }
