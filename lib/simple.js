...
END:VCARD`.trim()
    ...
    * text
    */
    conn.trans = async (text) => {
        return tr(text, { from: 'id', to: language }).catch((_) => tr2(text, { from: 'id', to: language }))
    }
    ...
    */
    ...
    conn.reply2 = async (jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text: await tr(text, {from: 'id', to: language }).catch(async _ => await tr2(text, {from: 'id', to: language })) }, { quoted, ...options })
    }
    conn.fakeReply = (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
        return conn.sendMessage(jid, { ...options, text: text }, { quoted: { key: { fromMe: fakeJid == conn.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
    }
    conn.fakeReply2 = async (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
        return conn.sendMessage(jid, { ...options, text: await tr(text, {from: 'id', to: language }).catch(async _ => await tr2(text, {from: 'auto', to: language })) }, { quoted: { key: { fromMe: fakeJid == conn.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
    }
