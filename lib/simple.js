...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.updateProfilePicture = async value(jid, buffer) {
    ...
    */
    ...
    conn.resize = async (PATH, uk1, uk2) => {
    	let buffer = Buffer.isBuffer(PATH) ? PATH : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (await fetch(PATH)).buffer() : fs.existsSync(PATH) ? (filename = PATH, fs.readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0) 
     ...
     conn.sendSticker = async value(jid, path, quoted, options = {}) {
