         ...
         switch (action) {
               ...
               case 'remove':
                     ...
                     for (let user of participants) {
                            let pp = 'https://telegra.ph/file/386c51ab2f53692fee20d.jpg'
                            try {
                                   pp = await this.getProfilePicture(user)
                                   } catch (e) {
                                   } finally {
                                       let jng = await this.getName(user)
           let text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, #user!').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
           (chat.sBye || this.bye || conn.bye || 'Bye, #user!')).replace('#user', '' + jng)
            })
                break
            case 'promote':
