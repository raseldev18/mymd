...
module.exports = {
    async handler(chatUpdate) {
        ...
        global.wait1 = '*「 ⏱️ 」Tunggu sebentar...*'
        global.eror1 = '*「 📡 」Maaf server down...*'
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    	conn.sendButtonLoc(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy or ${usedPrefix}buyall* atau klik button di bawah!`, wm, await(await fetch(img)).buffer(), [[`Buy`, `#buy`], [`Buyall`, `#buyall`]], m)
                       // sock.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, m)
                        continue // Limit habis
                    }
                    if (plugin.level > _user.level) {
                    	conn.sendButtonLoc(m.chat, `Diperlukan level *${plugin.level}* untuk menggunakan perintah ini. Level kamu *${_user.level},* naikan Levelmu dengan mengetik *${usedPrefix}levelup* atau klik button di bawah!`, wm, await(await fetch(img)).buffer(), [[`Levelup`, `#levelup`]], m)
